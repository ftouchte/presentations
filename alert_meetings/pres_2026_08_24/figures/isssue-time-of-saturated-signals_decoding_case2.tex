...
% % pente montante y = 20/3(x-3/4) so x = 3/20 y + 3/4
% % pente descendante -2.5x + 10 = y so x = (10-y)/2.5
...
\draw [black, name path = saturatedSignal] (0.5,0) .. controls (0.75,0) .. (21/20,2)
                     -- (8/2.5,2)
                     .. controls (3,2.5) and (4,0) .. (5,0);
...
\path [name path = saturatedThreshold, red] (0,1) -- (4, 1);
...
% draw time
\draw [dotted] (N1) -- (N1 |- 0,0) node [below right] {\scalebox{0.75}{reconstructed time}};
...
% draw ADC limits
\draw [dotted] (1.8,4.5) -- (0,4.5) node [left] {\scalebox{0.75}{estimated ADC}};

% Draw error band
\pgfmathsetmacro{\adcError}{0.2}
\pgfmathsetmacro{\timeError}{\adcError*3/20}
\fill [gray, fill opacity=0.4] (3,4.5+\adcError) rectangle (0,4.5-\adcError);

\draw [dotted] (N1) -- (0,2.25) node [left] {\scalebox{0.75}{1/2 estimated ADC}};
\fill [gray, fill opacity=0.4] ($(N1)+(0,\adcError)$) rectangle (0,2.25-\adcError);

\coordinate (N1x) at (N1 |- 0,0);
\fill [gray, fill opacity=0.4] ($(N1)+(-\timeError,0)$) rectangle ($(N1x)+(\timeError,0)$);
...
% draw slope
% \pgfmathsetmacro{\xslope}{1}
% \draw [red, name path = slopeVerticalLine] (\xslope, -1) -- (\xslope, 5); 
% \path [name intersections={of=normalSignal and slopeVerticalLine, by=SL}];

\coordinate (A) at (0.75,0);
\coordinate (B) at (1.5,5);
\coordinate (AA) at ($(A)!-0.25!(B)$);
\coordinate (BB) at ($(A)!1.25!(B)$);

\draw [dashdotted] (AA) -- (BB);

% draw step circle
\node [right, draw, circle, solid, thin, inner sep=2pt, xshift=2pt] at (BB) {\scalebox{0.75}{1}};


\node [right, draw, circle, solid, thin, inner sep=2pt, xshift=2pt] at (2.1,4.5) {\scalebox{0.75}{2}};

\node [right, draw, circle, solid, thin, inner sep=2pt, xshift=2pt] at (N1) {\scalebox{0.75}{3}};

\node [above right, draw, circle, solid, thin, inner sep=2pt, xshift=3pt, yshift=3pt] at (N1x) {\scalebox{0.75}{4}};

\coordinate (CC) at ($(A)!0.35!(B)$);
\fill [black] (CC) circle [radius=2pt];
