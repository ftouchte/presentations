...
\begin{frame}{Time reconstruction 1/2}
    \begin{itemize}
    ...
    {   \setlength{\tabcolsep}{2pt}
        ...
        \begin{tabular}{cc}
            ...
            \footnotesize 1/2 ADC below saturation & 
                \footnotesize 1/2 ADC below saturation \\
            \footnotesize (direct interpolation) & 
                \footnotesize (use the maximum slope to extrapolate)
        \end{tabular}
    }
\end{frame}

\begin{frame}{Time reconstruction 2/2}
    \begin{itemize}
        \item With the corrected amplitude, we can recompute the time
            \begin{itemize}
                \item We use the time at half the corrected ADC
    ...
    {   \setlength{\tabcolsep}{2pt}
        ...
        \begin{tabular}{cc}
            \includegraphics[width=0.48\linewidth]{figures/build/isssue-time-of-saturated-signals_decoding.pdf} &
                \includegraphics[width=0.48\linewidth]{figures/build/isssue-time-of-saturated-signals_decoding_case3.pdf} \\
            \footnotesize 1/2 ADC below saturation & 
                \footnotesize 1/2 ADC below saturation \\
            \footnotesize (direct interpolation) & 
                \footnotesize (use the previous slope to extrapolate)
        \end{tabular}
    }
\end{frame}
    ...
    \begin{itemize}
        ...
        \item After correction
            \begin{itemize}
                ...
                \item We move from 2244 reconstructed tracks to \sout{2436} 2478
                    \begin{itemize}
...
\begin{frame}{Pull request}
    \begin{itemize}
        \item The pull is ready for review \href{https://github.com/JeffersonLab/coatjava/pull/1397}{\underline{link}}
        \item Other changes
            \begin{itemize}
                \item A waveform qualified 1 cannot be assigned another type later on
                    \begin{itemize}
                        \item[$\rightarrow$] Before some of them were requalified as 2, because of their missing trailing edge
                    \end{itemize}
                \item The saturation limit move from 3500 to 3600 ADC
                \item The upper limit of the ADC cut move from 4095 to 1e10
                    \begin{itemize}
                        \item[$\rightarrow$] cf. \href{https://clasweb.jlab.org/cgi-bin/ccdb/show_request?request=/calibration/alert/ahdc/raw_hit_cuts:0:default:2026-08-24_06-05-24}{updated CCDB \underline{/calibration/alert/ahdc/raw\_hit\_cuts}}
                    \end{itemize}
            \end{itemize}
    \end{itemize}
